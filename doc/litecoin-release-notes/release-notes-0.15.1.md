Skrcoin Core version *0.15.1* is now available from:

  <https://download.skrcoin.org/skrcoin-0.15.1/>
...
A number of changes to the way Skrcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 coins per kB (0.00001 SKR). This is to prepare for dropping the minimum transaction fee to 0.1 coins per kB (0.0001 SKR) in the next release.
