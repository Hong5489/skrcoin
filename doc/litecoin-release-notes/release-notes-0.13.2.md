Skrcoin Core version 0.13.2 is now available from:

  <https://download.skrcoin.org/skrcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/skrcoin-project/skrcoin/issues>
...
updates will be released anymore. Without security updates, using a skrcoin
...
ECDSA signatures inside Skrcoin transactions now use validation using
...
Previous versions of Skrcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Skrcoin Core was to raise the default minimum
relay fee.

Skrcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Skrcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running skrcoind without having to do any manual configuration.
...
Skrcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Skrcoin Core will relay transactions with
...
entirely in Skrcoin Core 0.13, and it is at this time undecided whether the
...
Skrcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Skrcoin Core automatically creates a hidden service to listen on, without
manual configuration. Skrcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Skrcoin Core is listening, and
...
Skrcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Skrcoin
Core to use floating fees. By default, Skrcoin Core will use floating
fees.
...
(default: `0.0002` SKR/kB).

At all times, Skrcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SKR.
Furthermore, Skrcoin Core will never create transactions paying less than
...
`skrcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /skrcoinrpc>
        ...
        # optional bypass skrcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< skrcoinrpc:<password>
    </Location>
...
Restarting skrcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in skrcoin.conf
...
For example:
    ...
    $ src/skrcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Skrcoin Core code base has
...
- `skrcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `skrcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Skrcoin scripting system that in combination with
...
Skrcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Skrcoin Core.
...
The two phases are distinct in the Skrcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Skrcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Skrcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Skrcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of skrcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of skrcoins being spent is exactly
...
- **Increased security for multisig:** Skrcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Skrcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Skrcoin's current ledger (called the UTXO set),
  ...
  note that Skrcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Skrcoin users to individually opt-in to almost any change in the Skrcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Skrcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Skrcoin
client to Skrcoin Core 0.13.2 to signal for segwit support. When a
...
Since Skrcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `skrcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `skrcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
